...
const ENFORCED_BIBLE_TEXT_BOOKS = new Set(['genesis', 'exodus', 'leviticus', 'numbers', 'deuteronomy', 'joshua', 'judges', 'ruth', '1-samuel', '2-samuel'])
...
const hardBlockedTerms = [
  ...
  {
    pattern: /\b(ephah|hin|gerah|gerahs|homer|homers|omer|omers|talent|talents|bekah)\b/,
    message: 'Replace ancient measure names with simple measured amounts.',
  },
  {
    pattern: /\b(ephod|breastpiece|turban|sash|sashes)\b/,
    message: 'Replace priestly clothing terms with child-readable descriptions.',
  },
  {
...
function finalPolish(text) {
  return text
    ...
    .replace(/\ban bad report\b/g, 'a bad report')
    .replace(/\baccording as\b/g, 'as')
    .replace(/\bmake holy me\b/g, 'show that I am holy')
    .replace(/\bmust not sacrifice it to the Lord your God if it has any something wrong-is lame or blind, or has any something wrong whatever\b/g, 'must not sacrifice it to the Lord your God if it is lame, blind, or has anything wrong with it')
    ...
    .replace(/\bin the ears of the Lord\b/g, 'where the Lord heard them')
    .replace(/\bcomplaining where the Lord heard them\b/g, 'complaining where the Lord could hear')
    .replace(/\bconsumed the burned offering\b/g, 'burned up the burned offering')
    .replace(/\bconsumed some of\b/g, 'burned up some of')
    .replace(/\bconsume them in a moment\b/g, 'destroy them in a moment')
    .replace(/\bwill be consumed\b/g, 'will die')
    .replace(/\bthey will be consumed\b/g, 'they will die')
    .replace(/\bwill bear your unfaithfulness\b/g, 'will carry the result of your unfaithfulness')
    .replace(/\bbear your unfaithfulness\b/g, 'carry the result of your unfaithfulness')
    .replace(/\bwill bear your sins\b/g, 'will carry the guilt of your sins')
    .replace(/\bbear your sins\b/g, 'carry the guilt of your sins')
    .replace(/\bevil whole group\b/g, 'rebellious group')
    .replace(/\bof the flesh is half consumed\b/g, 'whose body is wasting away')
    .replace(/\bthe flesh is half consumed\b/g, 'the body is wasting away')
    .replace(/\binhabitants\b/g, 'people who live there')
